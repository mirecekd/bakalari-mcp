...
echo ""
echo "Usage examples (Local images):"
...
echo ""
echo "Usage examples (GHCR images):"
echo "  CLI (stdio):     docker run --rm -i ghcr.io/mirecekd/bakalari-mcp:latest-cli --user USER --password PASS --url URL"
echo "  Proxy (HTTP):    docker run -e BAKALARI_USER=USER -e BAKALARI_PASSWORD=PASS -e BAKALARI_URL=URL -p 8805:8805 ghcr.io/mirecekd/bakalari-mcp:latest-proxy"
echo "  HTTP Streaming:  docker run -p 8806:8806 ghcr.io/mirecekd/bakalari-mcp:latest-http --user USER --password PASS --url URL"
